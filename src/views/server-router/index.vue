<template>
 ...
 <!--      <el-button type="primary" @click="multiDelBtnClick">
    ...
    >
      ...
      <el-table-column align="center" prop="id" label="路由id" />     
      <el-table-column align="center" prop="uri" label="服务地址" />      
      <el-table-column align="center" prop="predicates" label="断言(路由判断)">
        <template #default="scope">
          <p v-for="(item,index) in scope.row.predicates" :key="index" > {{item.name}} = {{item.args}} </p>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="filters" label="过滤条件">      
        <template #default="scope">
          <p v-for="(item,index) in scope.row.filters" :key="index" > {{item.name}} = {{item.args}} </p>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="detailDialog" :title="dialogTitle" width="500px" :close-on-click-modal="false">
      <div class="detail-container rowBC">
        <div class="detail-container-item">路由地址：{{ detailData.routerAddress }}</div>
      </div>
      <div class="detail-container rowBC">
        <div class="detail-container-item">允许访问的角色：{{ detailData.role }}</div>
      </div>
      <template #footer>
