<template>
  ...
  >
    <el-form ref="refForm" label-width="150px" :inline="false" :model="subForm" :rules="formRules" class="pr-5">
      <el-form-item label="路由id" prop="id" :rules="formRules.isNotNull" label-position="left">
        <el-input v-model="subForm.id" class="widthPx-150" placeholder="路由id" />
      </el-form-item>
      ...
      <el-form-item label="服务地址" prop="uri" :rules="formRules.isNotNull" label-position="left">
        <el-input v-model="subForm.uri" class="widthPx-150" placeholder="服务地址" />
      </el-form-item>
      <el-form-item label="断言(路由判断)" prop="predicates" :rules="formRules.isNotNull" label-position="left">
        <el-input v-model="subForm.predicates" class="widthPx-150" placeholder="断言(路由判断)" />
      </el-form-item>
      <el-form-item label="过滤条件" label-position="left">
        <el-input v-model="subForm.filters" class="widthPx-150" placeholder="过滤条件" />
      </el-form-item>
      <el-form-item label="元数据" label-position="left">
        <el-input v-model="subForm.metadata" class="widthPx-150" placeholder="元数据" />
      </el-form-item>
    </el-form>
    <template #footer>
...
const insertReq = () => {    
  subForm.predicates = subForm.predicates.split(',')  
  if (subForm.filters.length > 0) {
    subForm.filters = subForm.filters.split(',')
  }  
  var metaData = {};
  var meta = subForm.metadata.split(',')
  for (var i = 0; i < meta.length; i++) {       
    var metai = meta[i].split('=');
    metaData[metai[0]] = metai[1]    
  }
  subForm.metadata = metaData  
  axiosReq({
    url: '/routes/dynamic/add',
    data: subForm,
    method: 'post', //--c
    bfLoading: true
  }).then(() => {
