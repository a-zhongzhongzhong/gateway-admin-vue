<template>
  <div class="scroll-y">
    ...
    >
      ...
      <el-table-column align="center" prop="name" label="开发者名称" />           
    ...
    <el-dialog v-model="detailDialog" :title="dialogTitle" width="500px" :close-on-click-modal="false">
      <div class="detail-container rowBC">
        <div class="detail-container-item">开发者名称：{{ detailData.name }}</div>
      </div>      
      <div class="detail-container rowBC">
        <div class="detail-container-item">命名空间：{{ detailData.namespace }}</div>
      </div>      
      <template #footer>
...
let selectPageReq = () => {
  ...
  let reqConfig = {
    url: 'http://localhost:9999/developer/list',
    method: 'get',
    data,
    isParams: true
  }
...
/* const multiDelBtnClick = () => {
  ...
  elConfirm('删除', `您确定要删除【${deleteNameTitle.slice(0, stringLength)}】吗`)
    .then(() => {
      ...
      axiosReq({
        url: `http://localhost:9999/developer/del/`,
        data,
        method: 'DELETE',
        bfLoading: true
      }).then((res) => {
...
let deleteByIdReq = (id) => {
  return axiosReq({
    url: 'http://localhost:9999/developer/del/'+id,
    //data: { id: id },
    isParams: true,
    method: 'delete',
    bfLoading: true
