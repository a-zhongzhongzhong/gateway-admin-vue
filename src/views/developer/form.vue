<template>
  ...
  >
    <el-form ref="refForm" label-width="150px" :inline="false" :model="subForm" :rules="formRules" class="pr-5">
      <el-form-item label="开发者名称" prop="name" :rules="formRules.isNotNull" label-position="left">
        <el-input v-model="subForm.name" class="widthPx-150" placeholder="开发者名称" />
      </el-form-item>
      ...
      <el-form-item label="命名空间名称" prop="namespace" :rules="formRules.isNotNull" label-position="left">
        <el-input v-model="subForm.namespace" class="widthPx-150" placeholder="命名空间名称" />
      </el-form-item>
    </el-form>
    <template #footer>
...
const insertReq = () => {
  ...
  console.log(data);
  axiosReq({
    url: 'http://localhost:9999/developer/add',
    data: data,
    method: 'post', //--c
    bfLoading: true
  }).then(() => {
